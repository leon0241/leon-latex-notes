...
\begin{dfn}[Retracts and Deformation Retracts]{dfn:retracts}{9}
	\begin{itemize-zero}
	    \item A \textbf{retract} of $X$ onto a subspace $A \subset X$ is a map $r : X \to A$ such that $r \rvert_{A} = \Id_{A}$. Equivalently, this is a map $r : X \to X$ such that $r^{2} = r$ and $r(X) = A$. Or, s.t. $ri = \Id_{A}$, where $i : A \to X$ is the inclusion map.
		\vspace{-3pt}
...
Compute the sign of the cts $w_{3} : \mathbb{S}^{1} \to \mathbb{RP}^{2}; (x, y) \mapsto [x : 0 : -y]$. Parametarise in terms of $\sin$ and $\cos$: $(x, y) \mapsto [x : 0 : -y]$ becomes $(\sin(t), \cos(t)) \mapsto [\cos(t) : 0 : -\sin(t)]$. Since $\cos(0) = 1$ and $-\sin(0) = 0$ for it be closed, $\cos(\lambda 2\pi t) = 1$ and $-\sin(\lambda 2\pi t) = 0$ for a path $w_{3}(\lambda)$. sign is $1$.
...
Does the inclusion $i : \mathbb{RP}^{1} \to \mathbb{RP}^{2} ; [x : y] \to [x : y : 0]$ admit a retraction?\newline No. Consider the fundamental groups $\pi_{1}(\mathbb{RP}^{1}) \cong \mathbb{S}^{1} \cong \mathbb{Z}$, and $\pi_{1}(\mathbb{RP}^{2}) \cong \mathbb{Z} / 2\mathbb{Z}$, along with the induced inclusion map $i_{\ast} : \pi_{1}(\mathbb{RP}^{1})\to \pi_{1}(\mathbb{RP}^{2})$  which sends $\mathbb{Z}$ to $\mathbb{Z} / 2\mathbb{Z}$. If there is a retract, i.e. $r \circ i = \Id_{\mathbb{RP}^{1}}$ then we would need to have $r_{\ast} \circ i_{\ast} = id_{\pi_{1}(\mathbb{RP}^{1})}$. However there is no such map $r_{\ast} : \mathbb{Z} / 2\mathbb{Z} \to \mathbb{Z}$, therefore no such retraction can exist.
