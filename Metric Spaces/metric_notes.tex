...
\begin{dfn}[Inner Product Spaces]{dfn:inner-product-space}{}
    Let $X$a be a real vector space. An \textit{inner product} on $X$ is a function that assigns to every pair $(x,y)\in X \times X $ a real number denoted by $\langle x,y \rangle$ and has the following properties
    \begin{itemize}
        \item $\langle x,x \rangle\ge 0$ and $\langle x,x \rangle = 0 \iff x = 0$
        \item $\langle x,y \rangle = \langle y,x \rangle$
...
\begin{xmp}[Discrete metric]{xmp:discrete-metric}{}
    ...
    \[d(x,y) = \begin{cases}
        ...
        1, x \ne y
    \end{cases}\]
    Example of metric space without norm or inner prod
\end{xmp}
