...
\begin{rcl}[Integral Domains and Generators]{rcl:generated-ideal}{}
    ...
    \vspace{-15pt}
    \begin{itemize-zero}
        \item \textbf{Principal ideals} are ideals of the form $\langle r \rangle$. A \textbf{principle ideal domain} is an integral domain where every ideal is principal.
        ...
        \item An element $u\in R$ is a \textbf{unit} if it has a multiplicative inverse, i.e. if $\langle u \rangle = R$. The units form a group $R^{\times}$ under multiplication.
...
\begin{xmp}[Rational Expressions]{xmp:rational-expressions}{2.3.2}
    ...
    \[\frac{f(t)}{g(t)}\]
    where $f(t),\,g(t)\in K[t]$ with $g\ne 0$. Two such expressions, $f_{1} /g_{1}$ and $f_{2} /g_{2}$ are regarded as equal if $f_{1}g_{2} = f_{2}g_{1}$ in $K[t]$. i.e. equivalence class. The set of rational expressions over $K$ is denoted $K(t)$
\end{xmp}
    ...
    \begin{itemize-tight}
        ...
        \item[\textbf{2.3.6})] Let $\phi : K \to L$ be a homomorphism between fields.
            \begin{enumerate}[leftmargin=0em]
                \vspace{-7pt}
                \setlength\itemsep{0em}
                \item For a subfield $K'$ of $K$, the image $\phi K'$ is a subfield of $L$
...
\begin{rcl}[Characteristic]{rcl:characteristic}{2.3.9}
    \vspace{-5pt}
    Let $R$ be any ring. There is a unique homomorphism $\chi : \mathbb{Z} \to R$ whose kernel is an ideal of the principal ideal domain $\mathbb{Z}$. Hence $\ker \chi = \langle n \rangle$ for a unique integer $n \ge 0$. We call this $n$ the \textbf{characteristic} of $R$, written as $\Char R$. So for $m\in \mathbb{Z}$, we have that $m \cdot 1_{R} = 0$ iff $m$ is a multiple of $\Char R$. Or equivalently,
    \[\Char R = \begin{cases}
...
\begin{lma}[Characteristic Properties]{lma:integral-domain-char}{2.3.C}
    \vspace{-5pt}
    \begin{enumerate}[leftmargin=3.5em]
        \setlength\itemsep{0em}
        \item[\textbf{2.3.11})] The characteristic of an integral domain is $0$ or a prime number.
...
\begin{dfn}[Field Extension]{dfn:field-extension}{4.1.1}
    Let $K$ be a field. An \textbf{extension} of $K$ is a field $M$ together with a homomorphism $\iota : K \to M$, where $K$ is the small field and $M$ is the large field. We write $M : K$ to mean that $M$ is an extension of $K$, not bothering to mention $\iota$.
\end{dfn}
...
\begin{dfn}[Generated Subfields]{dfn:generated-subfield}{4.1.4}
    For a field $K$, and $X$ a subset of $K$, the subfield of $K$ \textbf{generated by} $X$ is the intersection of all subfields of $K$ containing $X$.

    Let $F$ be the subfield of $K$ generated by $X$. $F$ contains $X$, and $F$ is also the \textit{smallest} subfield of $K$ containing $X$ (i.e. any subfield of $K$ containing $X$ contains $F$)
...
\begin{dfn}[Minimal Polynomial]{dfn:minimal-polynomial}{4.2.7}
    Let $M : K$ be a field extension and let $\alpha\in M$ be \textit{algebraic} over $K$. The \textbf{minimal polynomial} of $\alpha$ is the unique monic\footnote{Monic means that the highest order element has coefficient $1$.} polynomial satisfying \eqref{eq:annihilator}.

    \textbf{Warning}: This isn't defined over transcendentals, therefore some elements of $M$ might not have a minimal polynomial.
\end{dfn}
...
\begin{dfn}[]{dfn:minimal-polynomial-element}{4.3.1}
    Let $K$ be a field.
    \begin{enumerate-zero}
        \item Let $m\in K[t]$ be monic and irreducible. Write $\alpha\in K[t] /\langle m \rangle$ for the image of $t$ under the canonical homomorphism $K[t] \to K[t] / \langle m \rangle$. Then $\alpha$ has minimal polynomial $m$ over $K$, and $K[t] /\langle m \rangle$ is generated by $\alpha$ over $K$.
...
\begin{rem}[Repeated Root]{rem:repeated-root}{7.1.A}
...
\begin{dfn}[Separable Polynomial]{dfn:separable}{7.2.2}
    An irreducible polynomial over a field is \textbf{separable} if it has no repeated roots in its splitting field.

    Equivalently, an irreducible polynomial $f\in K[t]$ is separable if it splits into \textit{distinct} linear factors in $\SF_{K}(f)$:
    ...
    \textbf{Warning}: this only works for \textit{irreducible polynomials}.
\end{dfn}

\begin{dfn}[Formal Derivative]{dfn:formal-derivative}{7.2.6}
    Let $K$ be a field and let $f(t) = \sum_{i = 0}^{n} i_{i}t^{i}\in K[t]$. The \textbf{formal derivative} of $f$ is
    \[(Df)(t) = \sum_{i = 1}^{n} i a_{i}t^{i-1} \in K[t]\]
\end{dfn}

\begin{lma}[Basic Derivative Rules]{lma:derivative-fules}{7.2.7}
    Let $K$ be a field. Then
    \[D(f + g) = Df + Dg, \quad D(fg) = f \cdot Dg + Df \cdot g, \quad Da=0\]
    for all $f,\,g\in K[t]$ and $\alpha\in K$
\end{lma}

\begin{lma}[Repeated Roots]{lma:repeated-roots}{7.2.9}
    Let $f$ be a nonzero polynomial over a field $K$. The following are equivalent:
    \begin{enumerate-tight}
        \item $f$ has a repeated root in $\SF_{K}(f)$
        \item $f$ and $Df$ have a common root in $\SF_{K}(f)$
        \item $f$ and $Df$ have a nonconstant common factor in $K[t]$
    \end{enumerate-tight}
\end{lma}

\begin{ppn}[Inseparability of Zero]{ppn:inseparability-zero}{7.2.10}
    Let $f$ be an irreducible polynomial over a field. Then $f$ is inseparable iff $Df = 0$
\end{ppn}

\begin{crl}[Separability of Irreducibles]{crl:separability-irreducible}{7.2.11}
    Let $K$ be a field.
    \begin{enumerate-tight}
        \item If $\Char K = 0$ then every irreducible polynomial over $K$ is separable.
        \item If $\Char K = p > 0$ then an irreducible polynomial $f\in K[t]$ is inseparable iff
            \[f(t) = b_{0} + b_{1}t^{p} + \cdots + b_{r}t^{rp}\]
            for some $b_{0},\dots,b_{r}\in K$
    \end{enumerate-tight}

    In other words, the only irreducible polynomials that are inseparable are the polynomials in $t^{p}$ in characteristic $p$.
\end{crl}
...
\begin{lma}[Separable Further Extensions]{lma:separable-further-extensions}{7.2.16}
    Let $M : L : K$ be field extensions, with $M : K$ algebraic. If $M : K$ is separable then so are $M : L$ and $L : K$.
\end{lma}

\begin{ppn}[Splitting Field Isomorphisms]{ppn:splitting-isos}{7.2.17}
    Let $\phi : K \to K'$ be an isomorphism of fields, let $0 \ne f \in K[t]$, let $M$ be a splitting field of $f$ over $K$, and let $M'$ be a splitting field of $\phi_{\ast}f$ over $K'$. Suppose that the extension $M' : K'$ is separable. Then there are exactly $[M : K]$ isomorphisms $\phi : M \to M'$ extensing $\psi$.
\end{ppn}

\begin{thm}[Size of Galois Extensions]{thm:galois-extension-size}{7.2.18}
    $\lvert \Gal(M : K) \rvert = [M : K]$ for every finite normal separable extension $M : K$
\end{thm}

\begin{rem}[Fixed Field]{rem:fixed-field}{7.3.A}
    Write $\Aut(M)$ for the group of automorphisms of a field $M$. Then $\Aut(M)$ acts naturally on $M$. Given a subset $S$ of $\Aut(M)$, we can consider the set $\Fix(S)$ of elements of $M$ fixed by $S$.
\end{rem}

\begin{lma}[Fixed Field is a Subfield]{lma:fixer-subfield}{7.3.1}
    $\Fix(S)$ is a subfield of $M$, for any $S \subseteq \Aut(M)$.
\end{lma}

\begin{thm}[Size of Fixed Field]{thm:fixed-field-size}{7.3.3}
    Let $M$ be a field and $H$ a finite subgroup of $\Aut(M)$. Then $[M : \Fix(H)] \le \lvert H \rvert$.
    This is actually an equality.
\end{thm}

\begin{ppn}[Fixed Field Normal Extension]{ppn:fixed-field-normal-ext}{7.3.7}
...
\begin{rem}[Intermediate Field]{rem:intermediate-field}{8.1.A}
    Let $M : K$ be a field extension, with $K$ viewed as a subfield of $M$. An \textbf{intermediate field} of $M : K$ is a subfield of $M$ containing $K$. Write
...
\end{tikzcd}\]
with the bigger fields higher up. We also write
...
% https://q.uiver.app/#q=
\[\begin{tikzcd}[cramped]
	M && 1 \\
	{L_2} && {\Gal(M : L_2)} \\
	{L_1} && {\Gal(M : L_1)} \\
...
\end{tikzcd}\]
    ...
    \begin{enumerate-tight}
        ...
        \item For all $L\in \mathscr{F}$,
            \[L \subseteq \Fix(\Gal(M : L))\]
            For all $H\in \mathscr{G}$,
...
\[\begin{tikzcd}[cramped]
	...
	\arrow["\Fix", shift left, from=1-3, to=1-1]
\end{tikzcd}\]
    are called the \textbf{Galois correspondence} for $M : K$. This terminology is mostly used in the case where the functions are \textbf{mutually inverse}, meaning that
    \[L = \Fix(\Gal(M : L)), \quad H = \Gal(M : \Fix(H))\]
    for all $L\in \mathscr{F}$ and $H\in \mathscr{G}$. In both cases, the LHS is a subset of the RHS. But they are not always equal.

    If $\Gal(M : -)$ and $\Fix$ \textit{are} mutually inverse then they set up a one-to-one corresponence between $\mathscr{F}$ and $\mathscr{G}$.
\end{rem}

\begin{thm-s}[Fundamental Theorem of Galois Theory]{thm:the-theorem}{8.2.1}
...
\begin{dfn}[Solvability by Radicals]{dfn:solvability-by-radicals}{9.1.5}
    A nonzero polynomial over $\mathbb{Q}$ is \textbf{solvable by radicals} if all of its complex roots are radical.
\end{dfn}

\newpage
\begin{lma}[Rational Galois Group is Abelian]{lma:rational-galois-group-abelian}{9.1.6}
    For all $n \ge 1$, the group $\Gal_{\mathbb{Q}}(t^{n} - 1)$ is abelian.
\end{lma}

\begin{lma}[Splitting Galois Group is Abelian]{lma:splitting-galois-group-abelian}{9.1.8}
    Let $K$ be a field and $n\ge 1$. Suppose that $t^{n} - 1$ splits in $K$. Then $\Gal_{K}(t^{n} - a)$ is abelian for all $a\in K$.
\end{lma}

\begin{rem}[Path of a Solvable Polynomial]{rem:solvable-poly-path}{9.1.A}
...
\begin{dfn}[Solvable Extension]{dfn:solvable-extension}{9.2.1}
    ...
    such that $L_{i} : L_{i - 1}$ is normal and $\Gal(L_{i} : L_{i - 1})$ is abelian for each $i\in \{1,\dots,r\}$.
\end{dfn}

\begin{lma}[Solvable Galois and Extensions]{lma:solvable-galois-extensions}{9.2.4}
    Let $M : K$ be a finite normal separable extension. Then
    \[M : K \text{ is solvable } \iff \Gal(M : K) \text{ is solvable}\]
\end{lma}

\begin{lma}[Finite Normal Results]{lma:finite-normal-results}{9.2.6}
    ...
    \begin{enumerate-tight}
        ...
        \item If $K : K$ is finite and normal with abelian Galois group, then so is $LL' : L'$
    \end{enumerate-tight}
    % TODO: the diagram
\end{lma}

\begin{lma}[Iterated Subfields]{lma:iterated-subfields}{9.2.7}
...
\begin{lma}[\texorpdfstring{$\Qsol$}{Qsol} is a subfield of \texorpdfstring{$\mathbb{C}$}{C}]{lma:qsol-c}{9.2.8}
   Let $\Qsol$ be defined as
   \begin{multline*}
...
\begin{lma}[Powers in \texorpdfstring{$\Qsol$}{Qsol}]{lma:qsol-powers}{9.2.9}
    Let $\alpha\in \mathbb{C}$ and $n \ge 1$. If $\alpha^{n}\in \Qsol$ then $\alpha\in \Qsol$.
\end{lma}

\begin{ppn}[\texorpdfstring{$\Qrad$}{Qrad} and \texorpdfstring{$\Qsol$}{Qsol}]{ppn:qrad-qsol}{9.2.12}
    $\Qrad \subseteq \Qsol$. That is, every radical number is contained in some subfield of $\mathbb{C}$ that is a finite, normal, solvable extension of $\mathbb{Q}$.
\end{ppn}
...
\begin{lma}[Irreducible Polynomials and Degrees]{lma:irreducible-degree}{9.3.1}
    Let $f$ be an irreducible polynomial over a field $K$, with $\SF_{K}(f) : K$ separable. Then $\deg(f)$ divides $\lvert \Gal_{K}(f) \rvert.$
\end{lma}

\begin{lma}[Generating the Symmetric Group]{lma:generating-sn}{9.3.2}
    For $n\ge 2$, the symmetric group $S_{n}$ is generated by $(12)$ and $(12 \dots n)$.
\end{lma}

\begin{lma}[Isomorphism to Symmetric Group]{lma:galois-iso-sn}{9.3.3}
    Let $p$ be a prime number, and let $f\in \mathbb{Q}[t]$ be an irreducible polynomial of degree $p$ with exactly $p - 2$ real roots. Then $\Gal_{\mathbb{Q}}(f) \cong S_{p}$.
\end{lma}
...
\begin{lma}[Characteristic of a Finite Field]{lma:finite-field-char}{10.1.1}
    Let $M$ be a finite field. Then $\Char M$ is a prime number $p$, and $\lvert M \rvert = p^{n}$ where $n = [M : \mathbb{F}_{p}] \ge 1$.

    In particular, the order of a finite field is a prime power.
\end{lma}

\begin{lma}[Splitting Prime Polynomials]{lma:splitting-prime-poly}{10.1.5}
    Let $p$ be a prime number and $n \ge 1$. Then the splitting field of $t^{p^{n}} - t$ over $\mathbb{F}_{p}$ has order $p^{n}$.
\end{lma}

\begin{lma}[Prime Powers are Equal]{lma:prime-powers-equal}{10.1.6}
    Let $M$ be a finite field of order $q$. Then $\alpha^{q} = \alpha$ for all $\alpha\in M$.
\end{lma}

\begin{lma}[Every Finite Field Splits]{lma:every-finite-field-splits}{10.1.8}
    Every finite field of order $q$ is a splitting field of $t^{q} - t$ over $\mathbb{F}_{p}$
\end{lma}

\begin{thm}[Classification of Finite Fields]{thm:classification-of-finite-fields}{10.1.9}
    \begin{enumerate-tight}
...
\begin{ppn}[Cyclic Finite Subgroups]{ppn:cyclic-finite-subgroups}{10.2.1}
    For an arbitrary field $K$, every finite subgroup of $K^{\times}$ is cyclic. In particular, if $K$ is finite, then $K^{\times}$ is cyclic.
\end{ppn}

\begin{crl}[Finite Field Extensions are Simple]{crl:fin-field-ext-is-simp}{10.2.5}
    Every extension of one finite field over another is simple.
\end{crl}

\begin{crl}[Existence of Irreducibles]{crl:existence-of-irreducibles}{10.2.8}
    For every prime number $p$ and integer $n\ge 1$, there exists an irreducible polynomial over $\mathbb{F}_{p}$ of degree $n$.
\end{crl}

\begin{lma}[Properties of Finite Field Extensions]{lma:separability-fin-field}{10.3.2}
...
\newpage
\begin{ppn}[Frobenius Automorphism]{ppn:frobenius-auto}{10.3.3}
    Let $p$ be a prime and $n \ge 1$. Then $\Gal(\mathbb{F}_{p^{n}} : \mathbb{F}_{p})$ is cyclic of order $n$, generated by the Frobenius Automorphism of $\mathbb{F}_{p^{n}}$
\end{ppn}

\begin{ppn}[Uniqueness of Finite Subfield]{ppn:uniqueness-finite-subfield}{10.3.6}
    Let $p$ be a prime and $n \ge 1$. Then $\mathbb{F}_{p^{n}}$ has exactly one subfield of order $p^{m}$ for each divisor $m$ of $n$, and no others. It is
    \[\{\alpha \in \mathbb{F}_{p^{n}} : \alpha^{p^{m}} = \alpha\}\]
\end{ppn}

\begin{ppn}[Cyclic Galois Groups]{ppn:cyclic-galois}{10.3.8}
    Let $M : K$ be a field extension with $M$ finite. Then $\Gal(M : K)$ is cyclic of order $[M : K]$.
\end{ppn}
