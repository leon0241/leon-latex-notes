...
\begin{xmp}[M\"obius band]{xmp:mobius-band}{5.7}
	The M\"obius band $M$ can be be defined as
	\[M = I^{2} / \sim\]
...
\begin{xmp}[Relative Homotopies and Retracts]{xmp:relative-retract}{6}
	A strong deformation retarct of $X$ onto $A$ is a deformation retract such that the homotopy $h : i \circ r \simeq \Id_{X}$ is relative to $A$.
\end{xmp}
...
\begin{dfn}[Homotopic Path]{dfn:homotopic-path}{13}
	...
	\end{equation}
	\tcbline
	In particular, at any point $t\in I$ a relative homotopy $h$ defins a path $h_{t} := h(-, t) : I \to X$ with start $\alpha(0) = \beta(0)$ and end $\alpha(1) = \beta(1)$. If one omits the relative condition, the start and end points of $h_{t}$ would be allowed to vary.
\end{dfn}
