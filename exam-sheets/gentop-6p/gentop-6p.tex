...
\begin{dfn}[Open Covers and Compct Spaces]{dfn:open-cover-compact}{4.1}
\begin{enumerate-tight}
...
\begin{dfn}[Open Covers as Collections]{dfn:open-cover-collection}{4.2}
\begin{enumerate-tight}
...
\begin{lma}[Normal Complements]{lma:normal-complements}{4.37}
	Show that a space $X$ is normal iff for every closed $F \subseteq X$ and open $G \subseteq X$ with $F \subseteq G$, there exist open $G'$ and closed $F'$ such that
...
\begin{rem}[]{rem:connected-clopen}{5.2}
	$X$ is connected iff the only subsets of $X$ which are clopen are $\emptyset$ and $X$
\end{rem}

% Examples

\begin{thm}[Connectedness of \texorpdfstring{$\mathbb{R}$}{R}]{thm:r-is-conneted}{5.4}
	$\mathbb{R}$ with the usual topology is connected.
\end{thm}

\begin{thm}[Connectedness of Functions]{thm:connected-functions}{5.5}
	If $f: X \to Y $ is continuous and $X$ is connected, then $f(X)$ (with the subspace topology) is connected
\end{thm}

\begin{crl}[Connectedness is a Prop]{crl:connectedness-prop}{5.6}
	Connectedness if a topological property: If $X,\, Y$ are homeomorphic spaces, then $X$ is connected iff $Y$ is connected.
\end{crl}
...
\item If $X$ is a compact conneted space, where for every pair of distinct points $x,\, y\in X$ the complement $X \backslash \{x,y\}$ is disconnected, then $X$ is homeomorphic to the circle $\mathbb{S}_{1}$
\end{itemize-tight}
\end{rem}
...
\begin{ppn}[Connected Squeeze theorem]{ppn:connected-squeeze}{5.9}
	Let $A$ be a connected subsets of a topological space $X$ and suppose $A \subseteq B \subseteq \overline{A}$. Then $B$ is connected.
\end{ppn}

\begin{crl}[Connected Intervals]{crl:connected-intervals}{5.10}
	Every nonempty interval $I \subseteq \mathbb{R}$ is connected.
\end{crl}
...
\begin{dfn}[Fixed Points of Maps]{dfn:fixed-point-map}{5.13}
	A \textbf{fixed point} of a map $f : X \to X$ is an $x\in X$ such that $f(x) = x$.
\end{dfn}
...
\begin{thm}[Path Connectedness and Continuity]{thm:path-conn-cont}{5.18}
	Suppose $f : X \to Y$ is a continuous map between topological spaces and that $X$ is path-connected. Then $f(X)$ is path-connected as a subspace of $Y$.
\end{thm}

\begin{crl}[Path-Connectedness Property]{crl:path-connected-prop}{5.21}
	Path-connectedness is a topological property: If $X, Y$ are homeomorphic spaces then $X$ is path-connected iff $Y$ is path-connected.
\end{crl}

\begin{ppn}[Path-Connectedness and ID Spaces]{ppn:path-connected-id-spaces}{5.22}
	For any equivalence relation $\sim$ on a path-connected space $X$ the identification space $Y = X / \sim$ is path-connected.
\end{ppn}
...
\begin{thm}[Connected means Path-Connected]{thm:connected-path-connected}{5.25}
	...
	Any connected open subset $\Omega \subseteq \mathbb{R}^{n}$ is also path-connected.
\end{thm}

\begin{lma}[Connected Families]{lma:connected-families}{5.30}
	Let $A_{\lambda} \subseteq X$, $(\lambda\in\Lambda)$ be a family of connected subsets of a topological space $X$. Suppose $\bigcup_{\lambda\in\Lambda} A_{\lambda} \ne \emptyset$. Then $\bigcup_{\lambda\in\Lambda} A_{\lambda}$ is connected.
\end{lma}
...
\begin{ppn}[Invariance of Path Components]{ppn:path-compoment-invariance}{5.37}
	The cardinality of sets of of path compoments $\pi_{0}(X)$ of a topologicl space $X$ is a topological invariant, i.e. if $X$ and $Y$ are homeomorphic, there is a bijection between $\pi_{0}(X)$ and $\pi_{0}(Y)$.

	However, existence of a bijection between $\pi_{0}(X)$ and $\pi_{0}(Y)$ does \textit{not} necessarily imply that $X$ and $Y$ are homeomorphic.
\end{ppn}
...
\begin{thm}[Path Connectedness and Openness]{thm:path-connected-open}{5.41}
	Let $X$ be a topological space. Then $X$ is path connected iff $X$ is connected \textit{and} for all $x\in X$ there is an open path connected $V$ such that $x\in V$.
\end{thm}
