...
where $A$ and $B$ are disjoint nonempty open subsets of $X$
\end{enumerate-tight}
Connectedness is a \textbf{Topological Property}. See \ref{ppn:ppn:invariants}
\end{dfn}
...
\begin{ppn}[Topological Invariants]{ppn:invariants}{A}
	...
	\begin{itemize}
		...
		\item[\textbf{4.11})] If $X$ is Compact, then so is $Y$
...
\begin{ppn}[Hausdorff if...]{ppn:if-hausdorff}{E}
	\begin{itemize-tight}
		\item[\textbf{3.4})] Suppose $(X, \mathcal{T})$ is a Hausdorff topological space and suppose $A$ is a subspace. Then $A$ is Hausdorff.
		\item The one-point compactification $X^{\infty}$ of a space $X$ is Hausdorff iff $X$ is Hausdorff and locally compact.
	\end{itemize-tight}
\end{ppn}


\begin{ppn}[Continuous if..]{ppn:continuity-properties}{B}
	\begin{itemize}
...
\begin{ppn}[Compact if...]{ppn:then-compact}{4.3}
	\begin{itemize-tight}
...
\begin{ppn}[Connected if...]{ppn:if-connected}{C}
	\begin{itemize-tight}
...
\begin{ppn}[Path-Connected if...]{ppn:if-pathconnected}{D}
	\begin{itemize-tight}
