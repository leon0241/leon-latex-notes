...
\begin{dfn}[Cesaro and Abel Summations]{dfn:cesaro-abel}{5.10}
...
\begin{rcl}[Random identities and other shit]{rcl:random-identities}{E}
	\begin{itemize}[leftmargin=*]
		...
		\item \textbf{Mean Value Theorem}: If $f : [a,b]\to\mathbb{R}$, $a < b$ is continuous in $[a,b]$, differentiable in $(a, b)$ then $\exists c\in (a,b)$ s.t.
			...
			\[\lvert f(x) - f(a) \rvert = \lvert f'(c) \rvert \lvert x - a \rvert\]
			and you can randomly substitute $a = 0$ to get
			\[\lvert f(x) - f(0) \rvert = \lvert f'(c) \rvert \lvert x - 0 \rvert \implies \lvert f'(c) \rvert \lvert x \rvert\]
			also for some reason it shows
...
\begin{xmp}[Random functions to compare]{xmp:comparison-functions}{yippee}
