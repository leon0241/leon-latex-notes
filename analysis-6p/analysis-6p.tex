...
\section{Theorem Bank}


\begin{thm}[Is something Integrable?]{thm:integrable-theorems}{A}
	\textbf{Theorem 4.2 (Basic Properties of Lebesgue Integral)}: Suppose $f$ and $g$ are integrable on $I$, and $\alpha$ and $\beta$ 
	\begin{enumerate}
		\setlength\itemsep{0em}
		\item $\alpha f + \beta g$ is integrable on $I$ and
		\[\int_{I} (\alpha f + \beta g)=\alpha\int_{I} f+\beta\int_{I} g\]
	...
	Let $I$ be an interval and $E \subset I$ a coutnable set. $\chi_{E}$ is integrable on $I$ and $\int_{I}\chi_{E} = 0$
\end{thm}

% TODO: Add the inverse to continuity - also add uniform continuity definition
% - Triangle Inequality (and reverse tri ineq)
