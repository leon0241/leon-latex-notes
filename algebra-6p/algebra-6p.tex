...
\begin{dfn}[All about Ideals]{dfn:ideal}{3.4}
    \textbf{Def 3.4.7}: A subset $I$ of a ring $R$ is an \textbf{ideal}, $I \unlhd R$, if the following hold:
    \begin{enumerate}
        \setlength\itemsep{0em}
        \item $I \ne \emptyset$
...
\begin{dfn}[Big def-thm pairs]{dfn:def-thm-pairs}{2.2}
    ...
    \textbf{Def 2.2.9}: The \textbf{rank} of a matrix $A\in \Mat(n \times m; F)$, written $\rk A$, is the dim. of the subspace of $F^{n}$ generated by the columns of $A$, or same with the row. The row/column rank are the same. If the rank is equal to the no. of rows/columns, then the matrix has \textbf{full rank}
