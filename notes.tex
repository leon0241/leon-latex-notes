...
\begin{itemize}
    \setlength\itemsep{0em}
    \item \textbf{Squeeze Theorem:} Suppose that $(x_{n})$, $(y_{n})$, and $(w_{n})$ are real sequences
    \vspace{-5pt}
    \renewcommand\labelitemi{\tiny$\bullet$}
    \begin{itemize}
        \setlength\itemsep{0em}
...
\begin{thm}[Top 10 Series Limit Theorems]{thm:limit-theorems}{lots}
\renewcommand\labelitemi{\tiny$\bullet$}
\begin{itemize}
    \setlength\itemsep{0em}
    ...
    \begin{itemize}
        \setlength\itemsep{0em}
        \item If $(a_n)_{n\in\N}$ converges, then $a_n\to0$.
    ...
    \item \textbf{Limit Comparison Test:} Let $(a_n)_{n\in\N}$ and $(b_n)_{n\in\N}$ be two real sequences with $a_n\geq0$ and $b_n\geq0$ for all n. Assume that $a_n / b_n \to L$ for some $L\in(0,\infty)$. Then, $\sum_{n=1}^{\infty}  a_n$ converges iff $\sum_{n=1}^{\infty}  b_n$ converges. 
...
\begin{thm}[Taylor's Formula with Remainder]{thm:taylor-remainder}{5.5.1}
