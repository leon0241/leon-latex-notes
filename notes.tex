...
\begin{dfn}[Orbit, Stabilizer, and Fix]{def:orbit}{4.2.1}
    ...
    \begin{itemize}
        ...
        \item For every $x$ in $X$, the \textbf{orbit} of $x$ is defined by
        \[\text{Orb}_{G}(x)=\{g\cdot x \mid g\in G\}\]
        This is a subset of $X$
        \item For every $x$ in $X$, the \textbf{stabilizer} of $x$ is defined by
        \[\text{Stab}_{G}(x)=\{g\in G \mid g\cdot x = x\}\]
        This is a subgroup of $G$
        \item For every $g$ in $G$, the \textbf{fix} of $g$ is defined by
        \[\Fix (g) = \{x\in X\mid g\cdot x=x\}\]
        Let $G$ act on $X$, let $x\in X$ and set $H:=\Stab_{G}(x)$. If $y=g\cdot x$ for some $g\in G$, then
        \[\send_{x}(y)=gH\]
        \item Let $h\in H$ and $g\in G :=X$. The conjugate action is:
        \[\{h\cdot g := hgh^{-1}\}\]
